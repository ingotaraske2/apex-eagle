...
APEX Eagle is a React app that generates AI trade signals using live web search, institutional flow analysis, candlestick chart analysis, and an outcome-loop quality gate (Claude Sonnet agent + Claude Haiku grader). The watchlist is fully user-managed — add or remove tickers, organize them into your own categories, and persist portfolio value and risk factor across sessions. It runs as a web app you use manually, **and** as a scheduled Cloudflare Worker that emails you BUY signals every Monday and Wednesday at 07:00 CET.
...
- Outcome loop: Claude Sonnet generates signals, Claude Haiku grader verifies quality (up to 3 iterations)
...
5. Open the deployed app → sign in with Google → paste your Anthropic API key when prompted → start trading
...
| Anthropic API | Pay per use (~$0.05–0.20/run on Sonnet + Haiku) |
...
- **AI:** Claude Sonnet 4.5 (signals + enrichment) · Claude Haiku 4.5 (grader)
